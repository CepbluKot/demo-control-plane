...
| Поле | Тип | Описание |
|---|---|---|
| `logs_sql_templates` | `list[str]` | Список SQL-шаблонов. Первый — основной источник, остальные — дополнительные. Плейсхолдеры: `{last_ts}`, `{period_end}`, `{limit}`, `{raw_limit}` |
...
# Resume — продолжить с места остановки (см. раздел 16)
python run_pipeline.py --resume runs/2026-04-01T10-05-00/my-incident/2026-04-01T10-10-00
python run_pipeline.py --resume <artifact_dir> --resume-incident my-incident-name
```
...
### Несколько источников данных (LOGS_SQLS)

Все SQL-шаблоны хранятся в едином списке `LOGS_SQLS` в `run_pipeline.py`. DataLoader запрашивает их **независимо** с одним `last_ts`, объединяет и сортирует результаты в Python. Передаются в `PipelineConfig.logs_sql_templates`.

**Почему не UNION ALL:** UNION ALL — лишняя нагрузка на ClickHouse (межисточниковый merge на стороне БД). Независимые запросы: каждый использует свои индексы, работают без конкуренции, сортировка дешёвая в Python.

**Как это работает:**
1. За каждый тик пагинации DataLoader запускает все шаблоны с одним `{last_ts}`
2. Строки объединяются и сортируются по `timestamp` в Python
3. "Последняя страница" = ВСЕ источники вернули < page_size строк
4. Watermark обновляется из merged-sorted результата (двухрежимная логика: `rows[-1]["end_time"]` для промежуточных, `max(end_time)` для последней)

**Валидация при старте:** `_validate_sql_templates(LOGS_SQLS)` проверяет наличие обязательных плейсхолдеров и колонок во всех шаблонах. `SystemExit` если что-то не так.

**Требования к каждому SQL-шаблону:**
- Плейсхолдеры: `{last_ts}`, `{period_end}`, `{limit}`, `{raw_limit}`
- Обязательные колонки в SELECT: `timestamp` (= start_time), `end_time`, `raw_line`
- Рекомендуемые: `namespace`, `pod_name`

**Тегирование источника в `raw_line`:** добавь префикс чтобы LLM различал источники:
```
[2026-03-18 14:48:07 → 14:50:21] ×3  k8s-ns/my-pod  [EVT:BackOff] Back-off restarting container
```

**Текущие источники в run_pipeline.py:**
| Индекс | Таблица | Фильтр |
|---|---|---|
| `LOGS_SQLS[0]` | `raw_lm.log_k8s_containers_MT` | airflow + spark + flex + kube-system, только ошибки |
| `LOGS_SQLS[1]` | `raw_lm.log_k8s_events` | `reason IN ('BackOff', 'ImagePullBackOff', 'OOMKilling', ...)` |

Чтобы добавить третий источник — добавь элемент в список `LOGS_SQLS`. Чтобы отключить — удали элемент из списка.

---

## 15. Resume — продолжение с места остановки

Если пайплайн прервался — можно продолжить без перезапуска с нуля.

### Команда

```bash
python run_pipeline.py --resume <artifact_dir>
# artifact_dir — папка вида runs/{run_ts}/{incident_name}/{artifact_ts}/
# Можно передать и map/ напрямую — найдёт сам

python run_pipeline.py --resume runs/2026-04-01T10-05-00/flex-spark-.../2026-04-01T10-10-00
python run_pipeline.py --resume <path> --resume-incident flex-spark-imagepullbackoff-2026-04-01
# --resume-incident нужен только если несколько инцидентов в INCIDENTS
```

### Логика — что пропускается

Resume проверяет наличие выходных файлов и пропускает уже готовые шаги:

| Файл в artifact_dir | Если есть | Если нет |
|---|---|---|
| `reduce/final_merged.json` | REDUCE пропускается | REDUCE запускается по MAP-чанкам |
| `report_data.md` | программный отчёт пропускается | генерируется заново |
| `report_multipass.md` | multipass пропускается | генерируется заново |
| `report.md` | монолитный отчёт пропускается | генерируется заново |

**Идемпотентность:** resume можно запускать многократно — каждый раз делает только то, чего ещё нет. Последующие resume используют результаты предыдущих.

**Перегенерация конкретного шага:** удали нужный файл и запусти `--resume` снова.

**Частичный MAP:** если MAP прервался на середине — загрузятся только сохранённые чанки (`map/chunk_000.json`, `chunk_001.json`, ...). REDUCE будет работать с частичными данными.

### Логи resume

- Консоль (stderr) — как обычно
- `<artifact_dir>/pipeline_resume.log` — файл рядом с артефактами того же прогона

### Артефакты (дополнение к разделу 3)

```
{artifact_timestamp}/
  pipeline_resume.log    ← лог каждого resume-запуска (добавляется к существующему)
  map/chunk_NNN.json     ← BatchAnalysis, сохраняется MapProcessor по мере обработки
  reduce/
    final_merged.json    ← финальный MergedAnalysis; ключевой файл для resume
    round_NN_group_NN.json  ← промежуточные шаги REDUCE
  report_data.md         ← программный отчёт
  report_multipass.md    ← главный отчёт (14 секций)
  report.md              ← монолитный нарратив
```
...
## 16. Smoke-check перед изменениями
