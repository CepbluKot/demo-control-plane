...
| `extra_logs_sql_templates` | `[]` | Дополнительные SQL-шаблоны (e.g. K8s events). Запускаются независимо с тем же `{last_ts}`, результаты сортируются в Python |
...
### Несколько источников данных (extra_logs_sql_templates)

Чтобы добавить второй источник (например, `log_k8s_events`), используй `extra_logs_sql_templates` в PipelineConfig. DataLoader запрашивает все источники **независимо** с одним `last_ts`, объединяет и сортирует результаты в Python.

**Почему не UNION ALL:** UNION ALL — лишняя нагрузка на ClickHouse (межисточниковый merge на стороне БД, потребляет оперативу и CPU). Независимые запросы: каждый использует свои индексы, работают без конкуренции, сортировка дешёвая в Python.
...
**Требования к SQL-шаблону:**
- Те же плейсхолдеры: `{last_ts}`, `{period_end}`, `{limit}`, `{raw_limit}`
...
| Переменная | Таблица | Фильтр |
|---|---|---|
| `LOGS_SQL` | `raw_lm.log_k8s_containers_MT` | airflow + kube-system, только ошибки |
| `EVENTS_SQL` | `raw_lm.log_k8s_events` | reason IN (BackOff, Failed, OOMKilling, ...) |

Чтобы отключить events — очисти `EVENTS_SQL = ""`. Чтобы добавить третий источник — добавь его в список: `extra_logs_sql_templates=[EVENTS_SQL.strip(), ANOTHER_SQL.strip()]`.
