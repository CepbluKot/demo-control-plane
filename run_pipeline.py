...
FROM (
    ...
    FROM (
        ...
        FROM (
            ...
            WHERE timestamp BETWEEN parseDateTime64BestEffort('{period_start}')
                                AND parseDateTime64BestEffort('{period_end}')
...
-- Keyset-пагинация: {last_ts} обновляется DataLoader после каждой страницы.
-- Не используй {start_time} здесь — он константен и пагинация сломается.
WHERE start_time > parseDateTime64BestEffort('{last_ts}')
...
# ClickHouse держит весь {start_time}..{end_time} в памяти для вычисления функций,
# при большом периоде это приводит к OOM.
# 0 — без разбивки. Рекомендовано: 2.0 если период > 4 часов.
QUERY_TIME_SLICE_HOURS = 2.0
