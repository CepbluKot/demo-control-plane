...
LOGS_SQLS = [
    # ── Источник 1: Spark logs (k8s_logs.k-ndp-p11-ndp-flex-spark) ──────────
...
INCIDENTS = [
    {
        "name": "spark-periodic-driver-failures-no-kube-logs-2026-07",
        "context": """
            Можешь помочь с расследованием инцидента?
            ...
            Нужно отследить и другие подобные падения за период с 16.07 по 22.07
            и выявить закономерность или возможную причину по логам spark submit / executor.
        """,
        ...
        "incident_start": datetime(2026, 7, 16, 0, 0, 0, tzinfo=MSK),
        "incident_end":   datetime(2026, 7, 23, 0, 0, 0, tzinfo=MSK),
        ...
        "context_start": datetime(2026, 7, 16, 0, 0, 0, tzinfo=MSK),
        "context_end":   datetime(2026, 7, 23, 0, 0, 0, tzinfo=MSK),
    },
...
FREEFORM_START:   datetime | None = datetime(2026, 7, 16, 0, 0, 0, tzinfo=MSK)
FREEFORM_END:     datetime | None = datetime(2026, 7, 23, 0, 0, 0, tzinfo=MSK)
FREEFORM_CONTEXT: str             = """
    ...
    при этом в Kubernetes логов pod не обнаруживается.
    Такие кейсы не единичны, происходят периодически, задания могут падать абсолютно разные.
    Направлены дополнительные логи загрузки (spark submit + executor).
    Нужно с помощью инструмента отследить и другие подобные падения с похожей ошибкой
    за период с 16.07 по 22.07 и выявить закономерность или возможную причину,
    которую можно увидеть по логам.
    Если полный период окажется слишком тяжёлым, первым fallback-периодом считать 17.07.
"""
